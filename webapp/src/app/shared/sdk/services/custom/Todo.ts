...
export class TodoApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined): Observable<Todo> {
    ...
    return result.map((instance: Todo) => new Todo(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined): Observable<Array<Todo>> {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<Todo>) =>
        instances.map((instance: Todo) => new Todo(instance))
    );
  }
  ...
  public stats(range: any, custom: any = undefined, where: any = undefined, groupBy: any = undefined): Observable<any> {
    ...
    if (custom) urlParams.custom = custom;
    if (where) urlParams.where = where;
    if (groupBy) urlParams.groupBy = groupBy;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result;
  }
